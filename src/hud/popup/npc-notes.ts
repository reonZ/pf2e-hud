...
class NpcNotesHudPopup extends BaseHudPopup<NPCPF2e> {
    ...
    ): Promise<NpcNotesHudContext> {
        const actor = this.actor;
        const rollData = actor.getRollData();
        const system = (actor as NPCPF2e).system;
        const whitelist = Object.keys(CONFIG.PF2E.creatureTraits);

        const publicNotes = await ux.TextEditor.implementation.enrichHTML(
            system.details.publicNotes,
            { rollData, secrets: actor.isOwner }
        );

        const privateNotes = await ux.TextEditor.implementation.enrichHTML(
