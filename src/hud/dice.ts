...
class DicePF2eHUD extends FoundrySidebarPF2eHUD<DiceSettings> {
    static DICE = {
    ...
    async _prepareContext(options: ApplicationRenderOptions): Promise<DiceContext> {
        const dice = Object.entries(DicePF2eHUD.DICE).map(([face, path]) => {
            return {
                face,
                path,
                label: `d${face}`,
        ...
        return {
            dice,
            dten: dice[3].path,
            flats: [5, 11].map((dc) => ({ dc, label: flatCheckLabel(dc) })),
        };
    }
        ...
        addListenerAll(html, "[data-action]", (el, event) => {
            ...
            switch (action) {
                ...
                case "roll-flat": {
                    ...
                    const actor = R.only(canvas.tokens.controlled)?.actor ?? game.user.character;

                    game.pf2e.Check.roll(new game.pf2e.StatisticModifier(flatCheckLabel(dc), []), {
...
function flatCheckLabel(dc: number) {
