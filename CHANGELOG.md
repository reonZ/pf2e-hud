...
- `Spell Shortcut`:
  - fix slot and tooltip action cost icon
...
- `Strike Shortcut`:
  ...
  - fix slot and tooltip not showing the action cost icon
...
- `Foundry Sidebar`:
  - add new `Hide Format Panel` setting which removes the newly added format panel from the chat
  - update and slightly improve the `Hide Roll Modes`
...
- `Actions Sidebar`:
  - change the behaviour of the `use` button of actions with a self-applied effect
    - it now sends the item to chat and directly adds the effect instead of replicating the system's workflow
- `Elemental Blast Shortcut`:
  - using the shortcut to channel elements now directly applies the effect and sends the `Channel Elements` item to chat instead of replicating the system's workflow
- `Spell Shortcut`:
  - fix animist vessel shortcuts not being disabled for not being the primary vessel
